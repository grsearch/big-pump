...
test('C ignores low FDV and exits at 30m; no inherited fixed profit/stop',()=>{
  ...
  assert.equal(stonkExit({...p},null,diffusionDefaults,now+1800000),'持仓满 30 分钟');
...
test('C queues timeout with missing quote and only closes on subsequent valid data',()=>{
  ...
  run=diffusionStep(run,[],now+1816000);assert.match(run.arms[2].positions[0].pendingExit.reason,/30 分钟/);assert.equal(run.arms[2].positions[0].status,'open');
  const later=now+1832000;const coin=t({marketAt:later,execution:{...t().execution,at:later,priceAt:later}});
...
test('C upgrade retires pending 15 minute timeout and keeps the position until 30 minutes',()=>{
 let run=diffusionStep(newDiffusionRun(now),[t()],now);run.arms[2].positions[0].pendingExit={at:now+900000,reason:'持仓满 15 分钟'};
 const at=now+960000;run=diffusionStep(run,[t({marketAt:at,execution:{...t().execution,at,priceAt:at}})],at);
 assert.equal(run.arms[2].positions[0].status,'open');assert.equal(run.arms[2].positions[0].pendingExit,null);
});
