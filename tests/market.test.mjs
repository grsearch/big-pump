...
import {newToken,transition,defaults} from '../lib/engine.ts';
...
test('direct pool lookup can recover from failed CA batch request',async()=>{const {s,w}=setup(),original=globalThis.fetch;globalThis.fetch=async url=>String(url).includes('/latest/')?new Response(JSON.stringify({pairs:[pair()]})):new Response('{}',{status:503});try{await w.market();assert.equal(s.get('token',ca).fdv,2223);assert.equal(s.get('token',ca).marketError,null);}finally{globalThis.fetch=original;s.close();}});
test('missing LP does not create a valid market snapshot or zero liquidity',async()=>{const {s,w}=setup(),original=globalThis.fetch;globalThis.fetch=async url=>new Response(JSON.stringify(String(url).includes('/latest/')?{pairs:[pair({liquidity:null})]}:[pair({liquidity:null})]));try{await w.market();assert.equal(s.get('token',ca).marketAt,null);assert.equal(s.get('token',ca).lp,null);assert.match(s.get('token',ca).marketError,/缺少 FDV 或 LP/);}finally{globalThis.fetch=original;s.close();}});
