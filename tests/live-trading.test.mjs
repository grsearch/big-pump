...
test('C exits at 40 percent activation, 10 percent drawdown and 30 minutes',()=>{
 ...
 assert.equal(exitReason({strategy:LIVE_C,costLamports:100,openedAt:now},20,now+1799999),'固定止损 -20%');assert.match(exitReason({strategy:LIVE_C,costLamports:100,openedAt:now},20,now+1800000),/30 分钟/);
});
