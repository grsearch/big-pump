...
test('official candidate is enrolled only after Helius verifies migration, without resetting Pump cursor',async()=>{const s=new Store(':memory:');const w=new Worker(s,{ENABLE_STONK:'true'});s.put('config','migrationCursor',{signature:'pump-cursor'});s.put('stonk-candidate',ca,stonkCandidate(row(),now));w.rpc=async()=>fixture();try{assert.equal(s.get('token',ca),null);assert(await w.stonk.confirm('migration-signature'));const t=s.get('token',ca);assert.equal(t.source,'stonk');assert.equal(t.pool,pool);assert.equal(t.transferFeeBps,300);assert.equal(t.migrationSignature,'migration-signature');assert(t.shadowBlocked);assert.equal(s.get('config','migrationCursor').signature,'pump-cursor');const age=t.graduatedAt;await w.stonk.confirm('duplicate');assert.equal(s.all('token').length,1);assert.equal(s.get('token',ca).graduatedAt,age);}finally{s.close();}});
