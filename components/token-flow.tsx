...
export default function TokenFlow({token}:any){
 ...
 {!f?<p>等待成交样本。</p>:<><p>{f.sampleCount} 笔样本 · {(f.coverage?.jobs?.length||f.coverage?.before)?'历史补扫中（最新扫描独立运行）':f.coverage?.caughtUpAt?'已完成一轮扫描':'等待扫描'} · 未解析交换 {f.coverage?.unsupported??0} 笔{f.coverage?.error?' · '+f.coverage.error:''}</p>
