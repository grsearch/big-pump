...
export default function ShadowPanel({data,demo,online,busy,act}:any) {
  ...
  return <>
    ...
    <div className="notice">C：Stonk 毕业触发，首个有效行情立即模拟买入；不等待 15 秒，不设 FDV 买入门槛，不依赖 X；+40% 激活移动止盈、回撤 10% 退出，持仓满 15 分钟卖出。C 不使用 A/B 的固定止盈止损；单笔金额和模拟费用沿用实验设置。首次启用不追买旧币。<br/>X 按 15 / 30 / 60 / 120 秒分级；15 秒池最多 5 币，每币最多加速 3 分钟。持仓行情目标每 5 秒查询，受请求耗时、限流和行情源更新速度约束。C 买入无额外等待，A/B 买入及卖出仍按模拟延迟执行；均计入滑点、手续费及税费，缺行情不伪造成交。</div>
