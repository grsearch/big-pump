...
export default function WalletResearch({data,now,busy,disabled,onScan,onCopy}:{data:any;now:number;busy:boolean;disabled:boolean;onScan:(address:string)=>Promise<boolean>;onCopy:(address:string)=>Promise<void>}) {
  ...
  return <>
    <div className="section-intro"><h2>寻找早买、长持、有大赢单的盈利钱包</h2><p>AGE &gt; {rules.minWalletAgeDays} 天；近 7 天已实现净收益 &gt; {rules.minSevenDayProfitSol} SOL。至少一笔早买大赢单兑现 ≥{rules.bigWinMultiple} 倍，至少 {rules.holdFraction*100}% 买入数量实际持有 ≥{rules.bigWinHoldHours} 小时后卖出。原有最低样本要求继续适用。</p></div>
    <div className="panel"><div className="panel-head"><h2>钱包研究库</h2><span>只有全部门槛通过才计入聪明钱包</span></div>
      ...
      <div className="table-scroll"><table><thead><tr>{['钱包','核验结果','链上 AGE','近 1 天净收益','近 7 天净收益','7 天大赢单','样本 / 长持合格','历史覆盖'].map(x=><th key={x}>{x}</th>)}</tr></thead><tbody>{rows.map((w:any)=><tr key={w.address}>
        ...
        <td>{w.bigWins?.length??0}<details><summary>查看兑现证据</summary>{(w.bigWins??[]).map((p:any)=><div key={p.ca}><a href={'https://gmgn.ai/sol/token/'+encodeURIComponent(p.ca)} target="_blank" rel="noopener noreferrer">{p.ca.slice(0,6)}…{p.ca.slice(-5)}</a><small>{p.multiple.toFixed(2)} 倍 · {profit(p.profit)}</small><small>{(p.longFraction*100).toFixed(0)}% 买入数量实持 ≥{rules.bigWinHoldHours}h 后卖出</small></div>)}{!w.bigWins?.length&&<small>尚无符合门槛的卖出证据</small>}</details></td>
