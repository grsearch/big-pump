...
export default function WalletResearch({data,now,busy,disabled,onScan,onCopy,onHistory}:{data:any;now:number;busy:boolean;disabled:boolean;onScan:(address:string)=>Promise<boolean>;onCopy:(address:string)=>Promise<void>;onHistory:(action:string)=>Promise<boolean>}) {
  const [address,setAddress]=useState(''),[filter,setFilter]=useState('pending');
  ...
  return <>
    ...
    {data.walletDiagnostics&&<p className="notice">自动核验：{data.researchEnabled?'已开启':'未开启'} · 排队 {data.walletDiagnostics.queued} · 达分页上限 {data.walletDiagnostics.capped} · 近期兑换待解析（旧记录需重扫） {data.walletDiagnostics.unsupported}。未达标与数据不足分别列出，不将未知认作亏损。</p>}<div className="panel"><div className="panel-head"><h2>钱包研究库</h2><span>只有全部门槛通过才计入聪明钱包</span></div>
      ...
      <div className="table-scroll"><table><thead><tr>{['钱包','核验结果','链上 AGE','近 1 天净收益','近 7 天净收益','7 天大赢单','样本 / 长持合格','历史覆盖'].map(x=><th key={x}>{x}</th>)}</tr></thead><tbody>{rows.map((w:any)=><tr key={w.address}>
        ...
        <td className={w.oneDayProfitSol==null?'':w.oneDayProfitSol>=0?'green':'negative'}>{profit(w.oneDayProfitSol)}{w.oneDayProfitSol==null&&w.knownOneDayProfitSol!=null&&<small>已核算部分 {profit(w.knownOneDayProfitSol)}</small>}</td>
        <td className={w.sevenDayProfitSol==null?'':w.sevenDayProfitSol>rules.minSevenDayProfitSol?'green':'negative'}>{profit(w.sevenDayProfitSol)}{w.sevenDayProfitSol==null&&w.knownSevenDayProfitSol!=null&&<small>已核算部分 {profit(w.knownSevenDayProfitSol)}</small>}</td>
        ...
        <td>{w.samples} / {w.wins}</td><td className="wrap">{w.coverageNote}{w.unresolvedSwapCount>0&&<small className="amber">近7天 {w.unresolvedSwapCount} 笔兑换待解析</small>}{w.unknownRealizedAssets?.length>0&&<small className="amber">近7天 {w.unknownRealizedAssets.length} 个币的卖出成本待核验</small>}{w.scanError&&<small className="amber">{w.scanError}</small>}{w.unknownPositions>0&&<small className="amber">{w.unknownPositions} 个币成本或汇率不完整</small>}</td>
      </tr>)}</tbody></table></div>{!rows.length&&<div className="empty-note">{filter==='verified'?'尚无通过全部门槛的钱包。可在“待核验 / 未达标”查看进度与原因。':'暂无待核验钱包。'}</div>}
    </div><div className="notice spaced">AGE 是最早已知链上活动证明的年龄下限。收益按卖出时间归入滚动 1 天 / 7 天，使用窗口前买入成本；不计未实现浮盈。历史、成本或 SOL 口径不完整则不验证。已核算部分不是完整收益，不用于冒充达标；不因年轻而隐藏观察记录。标签不会回填到过去的信号。</div>
  </>;
}
