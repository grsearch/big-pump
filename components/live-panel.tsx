...
export default function LivePanel({data,demo,online,busy,act}:any) {
  ...
  return <><div className="section-intro"><h2>A · 新增作者即买 · 实盘</h2><p>每次 0.1 SOL，Stonk 使用 Jupiter 路由。独立账本，不继承 Shadow 持仓或历史信号。</p></div>
    <div className="notice">{live?.acceptEntries?'新开仓已启用':'新开仓关闭'} · {live?.enabled?'服务器允许实盘':'服务器实盘开关关闭'} · {live?.configured?'钱包已配置':'钱包未就绪'}<br/>+100% 激活移动止盈，回撤 20% 全仓退出；无固定止盈止损，最长持仓 30 分钟。阈值依据扣费后的卖出报价，成交金额可能不同。</div>
    <div className="panel"><div className="panel-head"><h2>运行状态</h2><span>Jupiter {live?.jupiter?.used??0} / {live?.jupiter?.limit??60} 次 / 分钟</span></div><div className="detail-box">
      ...
      <p>单笔滑点上限 {(live?.slippageBps??100)/100}% · 网络费用和租金上限 {sol(live?.maxFeeLamports??5000000)}（不含 0.1 SOL 买入本金）</p>
    ...
    <div className="panel"><div className="panel-head"><h2>实盘持仓与收益</h2></div><div className="table-scroll"><table><thead><tr><th>Token / CA</th><th>状态</th><th>投入</th><th>卖出报价 / 已回收</th><th>已实现</th><th>退出条件 / 成交</th></tr></thead><tbody>{positions.map((p:any)=><tr key={p.ca}><td><a href={'https://gmgn.ai/sol/token/'+p.ca} target="_blank" rel="noreferrer">{p.symbol||p.ca.slice(0,8)} ↗</a></td><td>{p.status==='open'?'持仓中':'已平仓'}</td><td>{sol(p.costLamports)}</td><td>{sol(p.status==='closed'?p.proceedsLamports:p.markLamports)}<small>{p.quoteError|| (p.quoteAt?new Date(p.quoteAt).toLocaleTimeString():'')}</small></td><td>{sol(p.realizedLamports)}</td><td>{p.exitReason||'等待退出条件'}<small>{tx(p.sellSignature||p.buySignature)}</small></td></tr>)}</tbody></table></div>{!positions.length&&<div className="empty-note">尚无链上确认的实盘持仓。配置并手动启用后，只处理新的实时信号。</div>}</div>
    <div className="panel"><div className="panel-head"><h2>订单与跳过原因</h2><span>最近 100 条</span></div>{orders.slice().reverse().slice(0,100).map((o:any)=><div className="detail-box" key={o.id}><b>{o.side==='buy'?'买入':'卖出'} · {o.symbol||o.ca.slice(0,8)} · {labels[o.status]||o.status}</b><p>{o.reason}</p><small>{new Date(o.at).toLocaleString()} {o.signature&&tx(o.signature)}</small></div>)}{!orders.length&&<div className="empty-note">等待启用后的新增作者信号。</div>}</div>
  </>;
}
