...
export default function LivePanel({data,demo,online,busy,act}:any) {
  ...
  return <><div className="section-intro"><h2>C · Stonk 毕业即买 · 实盘</h2><p>每次 0.1 SOL，SOL 计价池使用 Raydium CPMM SDK 直连；非 SOL 池或直连失败时使用 Jupiter 后备。无额外链上模拟；不要求 X 热度或 FDV 门槛，不继承 Shadow 持仓或历史信号。旧 A 记录保留，旧持仓仍按原规则管理。</p></div>
    ...
    <div className="panel"><div className="panel-head"><h2>运行状态</h2><span>CPMM 直连 · Jupiter 后备 {live?.jupiter?.used??0} / {live?.jupiter?.limit??60} 次 / 分钟</span></div><div className="detail-box">
