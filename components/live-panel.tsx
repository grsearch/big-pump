'use client';
import MinuteEma from './minute-ema';
...
function Cashback({amount}:any){return amount>0?<small>另收返现 {(amount/1e9).toFixed(9)} SOL（单列）</small>:null;}
export default function LivePanel({data,demo,online,busy,act}:any) {
  ...
  const reconciliationIssues=orders.filter((o:any)=>o.status==='confirming'&&o.reconcileRequired);
  ...
  return <><div className="section-intro"><h2>C · Stonk 毕业即买 · 实盘</h2><p>每次 0.1 SOL，Stonk 使用 Jupiter 路由。使用 Jupiter 即时报价，取消本程序额外的链上模拟；不要求 X 热度或 FDV 门槛，不继承 Shadow 持仓或历史信号。旧 A 记录保留，旧持仓仍按原规则管理。</p></div>
    <div className="notice">{live?.acceptEntries?'新开仓已启用':'新开仓关闭'} · {live?.enabled?'服务器允许实盘':'服务器实盘开关关闭'} · {live?.configured?'钱包已配置':'钱包未就绪'}<br/>+40% 激活移动止盈，回撤 10% 全仓退出；固定止损 -20%、无固定止盈，最长持仓 30 分钟。阈值依据扣费后的卖出报价，成交金额可能不同。</div>
    ...
    {reconciliationIssues.length>0&&<section className="panel"><div className="panel-head"><h2>对账异常 · {reconciliationIssues.length} 笔</h2><span>待核对资产未计入持仓收益</span></div><div className="detail-box"><p className="amber">这些订单阻挡新开仓，程序会继续重试对账，不会重新发送原交易。已入账持仓仍继续管理。</p>{reconciliationIssues.map((o:any)=><div key={o.id} className="detail-box"><b>{symbol(o)} · {o.side==='buy'?'买入':'卖出'} · {o.reconciliationEvidence?.chainSuccess?'链上已成功，账务待核对':'对账失败'}</b><p>{o.reconcileError}</p>{o.reconciliationEvidence&&<><p>钱包 SOL 净变化：{signed(o.reconciliationEvidence.solDelta)}（不是买入成本）</p><p>目标币数量变化：{o.reconciliationEvidence.targetDelta}（原始单位）</p></>}{o.side==='buy'&&<p className="amber">这笔买入尚未建立持仓，不受自动止盈止损管理；需要核对实际资金流。</p>}<p>{tx(o.signature)} · 最近核对 {stamp(o.reconcileErrorAt)} · 失败 {o.reconcileFailures??0} 次</p>{o.reconciliationEvidence&&<details><summary>钱包资产变化证据</summary><pre>{JSON.stringify(o.reconciliationEvidence,null,2)}</pre></details>}</div>)}</div></section>}
    <section className="panel live-holdings">
      ...
      {open.slice().sort((a:any,b:any)=>b.openedAt-a.openedAt).map((p:any)=>{
        ...
        return <tr key={p.ca}>
          ...
          <td>{sol(v.cost)}<Cashback amount={p.buyCashbackLamports}/></td>
          ...
          <td className="live-reason">{selling?'卖出已提交，等待链上确认':p.exitReason|| (p.trailingActive?'移动止盈已激活':'持仓中，等待退出条件')}<small>{tx(p.buySignature)}</small><details><summary>1 分钟 EMA9 / EMA20</summary><MinuteEma token={data?.tokens?.find((t:any)=>t.ca===p.ca)} now={now}/></details>{p.quoteError&&<details><summary>报价错误</summary>{p.quoteError}</details>}</td>
        </tr>;
      })}</tbody></table></div>
      {!open.length&&<div className="empty-note">当前没有持仓。买入经链上确认后会显示在这里。</div>}
      <p className="live-footnote">浮动收益 = 最新卖出报价扣除预计网络费用 − 实际投入；不含 X / AI 研究费用。已识别的 PumpSwap 累积返现单独记录，不冲减买入成本、不计入本笔交易收益。超过 30 秒、请求失败或断线的报价不计为当前收益。实际成交可能不同。</p>
    </section>
    <section className="panel live-history">
      ...
      {history==='closed'?<><thead><tr><th>平仓时间 / 买入时间</th><th>策略目标币</th><th>投入</th><th>实际回收</th><th>已实现收益 / 收益率</th><th>退出原因 / 成交</th></tr></thead><tbody>{rows.map((p:any)=><tr key={p.ca}>
        <td>{stamp(p.closedAt)}<small>买入 {stamp(p.openedAt)}</small></td><td><a href={'https://gmgn.ai/sol/token/'+p.ca} target="_blank" rel="noreferrer">{symbol(p)} ↗</a><small>{p.ca.slice(0,6)}…{p.ca.slice(-6)}</small><small>{strategyName(p.strategy)}</small></td><td>{sol(p.costLamports)}<Cashback amount={p.buyCashbackLamports}/></td><td>{sol(p.proceedsLamports)}<Cashback amount={p.sellCashbackLamports}/></td><td className={tone(p.realizedLamports)}>{signed(p.realizedLamports)}<small>{Number.isFinite(p.realizedLamports)&&p.costLamports>0?`${(p.realizedLamports/p.costLamports*100).toFixed(2)}%`:'—'}</small></td><td className="live-reason">{p.exitReason||'—'}<small>{tx(p.sellSignature)}</small><small>{p.buySignature&&<a href={'https://solscan.io/tx/'+p.buySignature} target="_blank" rel="noreferrer">买入成交 ↗</a>}</small><TradeRoute record={p}/><details><summary>1 分钟 EMA9 / EMA20</summary><MinuteEma token={data?.tokens?.find((t:any)=>t.ca===p.ca)} now={now}/></details></td>
        ...
        <td>{stamp(o.confirmedAt??o.at)}</td><td><a href={'https://gmgn.ai/sol/token/'+o.ca} target="_blank" rel="noreferrer">{symbol(o)} ↗</a><small>{o.ca.slice(0,6)}…{o.ca.slice(-6)}</small><small>{strategyName(o.strategy)}</small></td><td>{o.side==='buy'?'买入':'卖出'}</td><td>{o.reconcileRequired?'账务待核对':labels[o.status]||o.status}</td><td>{o.receipt?.failed?sol(o.receipt.feeLamports):Number.isFinite(o.receipt?.solDelta)?sol(Math.abs(o.receipt.solDelta)):'—'}<small>{o.receipt?.failed?'链上失败手续费':o.status==='confirmed'?'交易净金额':'尚未确认成交'}</small></td><td className="live-reason">{o.reason||'—'}{o.attempts&&<small>尝试 {o.attempts} 次{o.status==='retrying'&&o.nextAttemptAt?` · 下次 ${new Date(o.nextAttemptAt).toLocaleTimeString()}`:''}</small>}{o.signature&&<small>{tx(o.signature)}</small>}<TradeRoute record={o}/>{o.receipt?.cashbackLamports>0&&<details><summary>返现与资金流</summary><p>钱包 SOL 净变化：{signed(o.receipt.walletSolDelta)}</p><p>本笔交易净变化：{signed(o.receipt.solDelta)}</p><Cashback amount={o.receipt.cashbackLamports}/><p>累积返现可能来自之前的交易，不计入本笔收益。</p><pre>{JSON.stringify(o.receipt.cashbackEvidence,null,2)}</pre></details>}{o.diagnostic&&<details><summary>错误详情</summary><pre>{JSON.stringify(o.diagnostic,null,2)}</pre></details>}</td>
      </tr>)}</tbody></>}
      </table></div>
