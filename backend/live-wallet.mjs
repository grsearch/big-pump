...
export class LiveWallet {
  ...
  async prepare(q, side, maxFeeLamports) {
    ...
    if (side === 'buy') {
      if (balance - afterBalance > Number(q.inAmount) + maxFeeLamports || afterQuantity - prior < BigInt(q.otherAmountThreshold)) throw Error('模拟买入金额或到账数量不符合报价');
    } else if (prior - afterQuantity !== BigInt(q.inAmount) || BigInt(afterBalance - balance + fees) < BigInt(q.otherAmountThreshold)) throw Error('模拟卖出金额不符合报价');
