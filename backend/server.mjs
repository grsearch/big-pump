...
const cleanup=setInterval(()=>{try{removePumpObservations(store);store.purgeExpired();}catch{}},60000);cleanup.unref();server.on('close',()=>{clearInterval(cleanup);clearInterval(liveTimer);});
