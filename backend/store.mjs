...
import {redact,tokenStreams,walletAudit} from './audit.mjs';
...
export class Store {
 ...
 put(kind,id,data){const prior=this.get(kind,id);if(kind==='token'&&data){const before=prior?tokenStreams(prior):{};for(const [stream,value] of Object.entries(tokenStreams(data)))if(JSON.stringify(value)!==JSON.stringify(before[stream]))this.audit(stream,id,value);}else if(kind==='wallet'){const summary=walletAudit(data);if(JSON.stringify(walletAudit(prior))!==JSON.stringify(summary))this.audit('wallet',null,{id,...summary});}else if(['live-order','live-position','event','ai-result','coverage'].includes(kind)&&JSON.stringify(prior)!==JSON.stringify(data))this.audit(kind,data?.ca??null,{id,...data});if(['token','ai-latest'].includes(kind)&&!this.get('token',id)&&this.get('token-reference',id))return data;this.db.prepare('INSERT INTO records VALUES (?,?,?) ON CONFLICT(kind,id) DO UPDATE SET data=excluded.data').run(kind,id,JSON.stringify(data));return data;}
