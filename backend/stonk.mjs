...
export class StonkDiscovery{
 ...
 enroll(found,signature){if(this.s.get('token',found.ca))return;const c=this.s.get('stonk-candidate',found.ca);this.w.enroll(found,null);const t=this.s.get('token',found.ca);if(!t)return;this.s.put('token',found.ca,{...t,...found,migrationSignature:signature,...(c?this.metadata(c):{}),smartCoverage:'暂未统计非 SOL 计价交易',shadowBlocked:'Stonk 税费与计价适配待完成，暂不参与 Shadow'});this.s.event('migration','Stonk 链上迁移已验证',found.ca);}
